...
export function createMap(
  ...
  map.on("popupclose", (e) => {
    const source = e.popup._source as L.CircleMarker | undefined;
    if (!source) return;
    const stored = markerTooltips.get(source);
    if (stored) source.bindTooltip(stored.html, stored.options);
  });
  ...
  function applyMarkerZoomScale(): void {
    ...
    const resizeMarker = (marker: L.CircleMarker) => {
      ...
      const visualRadius = abs
        ? Math.max(3, absoluteStationRadius(count, max, s.capacity) * zoomScale)
