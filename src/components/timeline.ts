...
function defaultHour(index: HistoryIndex | null, dayType: DayType, latestUpdated?: string | null): number {
  const hours = hoursForDayType(index, dayType, { latestUpdated });
...
function paintTimeline(container: HTMLElement, opts: TimeSelectorOptions) {
  const { index, timeView, latestUpdated } = opts;
  const isLatest = timeView.kind === "latest";
  const selectedDayType = timeView.kind === "hour" ? timeView.dayType : "weekday";
  const selectedHour =
    timeView.kind === "hour" ? timeView.hour : defaultHour(index, selectedDayType, latestUpdated);
  const hours = isLatest ? [] : hoursForDayType(index, selectedDayType, { latestUpdated });
    ...
    <section class="timeline">
      ...
      <div class="time-controls">
        ...
        <div class="day-type-toggle" role="group" aria-label="Tipus de dia">
          ${DAY_TYPES.map(
            (d) =>
              `<button type="button" class="day-type-btn ${!isLatest && selectedDayType === d.id ? "active" : ""}" data-day-type="${d.id}" ${hoursForDayType(index, d.id, { latestUpdated }).length === 0 ? 'title="Encara no hi ha dades"' : ""}>${d.label}</button>`
          ).join("")}
        </div>
        <label class="hour-select-label">
          Mitjana a les
          <select id="hour-select" class="time-select" ${isLatest || !hours.length ? "disabled" : ""}>
            ${isLatest ? `<option value="">—</option>` : hourOptions(index, selectedDayType, selectedHour, latestUpdated)}
          </select>
        </label>
      </div>
...
function readHour(container: HTMLElement, dayType: DayType): number {
  ...
  return defaultHour(timelineState?.index ?? null, dayType, timelineState?.latestUpdated);
}
...
function syncTimelineControls(container: HTMLElement, opts: TimeSelectorOptions) {
  ...
  const selectedHour =
    timeView.kind === "hour" ? timeView.hour : defaultHour(index, selectedDayType, latestUpdated);
  const hours = hoursForDayType(index, selectedDayType, { latestUpdated });
  const playing = opts.replayPlaying ?? false;
  const speed = opts.replaySpeed ?? 1;
  ...
  if (select) {
    ...
    if (!isLatest) {
      select.innerHTML = hourOptions(index, selectedDayType, selectedHour, latestUpdated);
    }
  }
  ...
  container.addEventListener("click", (e) => {
    ...
    if (dayBtn?.dataset.dayType) {
      ...
      const hours = hoursForDayType(timelineState.index, dayType, {
        latestUpdated: timelineState.latestUpdated,
      });
      ...
      const picked =
        hours.includes(hour)
          ? hour
          : defaultHour(timelineState.index, dayType, timelineState.latestUpdated);
...
function applyTimelineState(opts: TimeSelectorOptions): void {
  timelineState = {
    index: opts.index,
    view: opts.timeView,
    latestUpdated: opts.latestUpdated,
...
export function historicalStatusLabel(view: TimeView, playing: boolean): string {
  if (view.kind !== "hour") return "";
  const scope = `${dayTypeLabel(view.dayType)} · ${formatHour(view.hour)}`;
  return playing ? `▶ Replay · ${scope}` : scope;
}

export function replayStatusLabel(view: TimeView, playing: boolean): string {
  return historicalStatusLabel(view, playing);
}
