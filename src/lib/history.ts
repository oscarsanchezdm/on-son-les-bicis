...
const hourlyInflight = new Map<string, Promise<HourlySnapshot | null>>();
const hourlyViewCache = new Map<string, HourlyViewData>();
const hourlyViewInflight = new Map<string, Promise<HourlyViewData>>();

function hourlyViewCacheKey(hour: number, dayType: DayType): string {
  return `${dayType}:${hour}`;
}

export function clearHourlyViewCache(): void {
  hourlyViewCache.clear();
  hourlyViewInflight.clear();
}
...
async function loadHourlySnapshot(key: string): Promise<HourlySnapshot | null> {
  ...
  const inflight = hourlyInflight.get(key);
  if (inflight) return inflight;

  const promise = (async (): Promise<HourlySnapshot | null> => {
  ...
  hourlyInflight.set(key, promise);
  return promise;
}
...
export async function loadHistoryIndex(cacheKey?: string | null): Promise<HistoryIndex | null> {
  const qs = cacheKey ? `?v=${encodeURIComponent(cacheKey)}` : "";
  const res = await fetch(`${BASE}data/history/history-index.json${qs}`);
  if (!res.ok) return null;
  return res.json();
}
...
export function dayTypeFromMadridWeekday(weekday: string): DayType {
...
export function currentMadridDayType(): DayType {
  const weekday = new Intl.DateTimeFormat("en-US", {
    timeZone: MADRID_TZ,
    weekday: "short",
  }).format(new Date());
  return dayTypeFromMadridWeekday(weekday);
}

function madridHourAndDayTypeFromIso(iso: string): { hour: number; dayType: DayType } | null {
  try {
    ...
    return { hour, dayType: dayTypeFromMadridWeekday(weekday) };
  } catch {
    return null;
  }
}

export type HoursForDayTypeOpts = {
  /** Dades actuals: inclou la franja de l'últim snapshot si encara no és a l'índex. */
  latestUpdated?: string | null;
};

export function hoursForDayType(
  index: HistoryIndex | null,
  dayType: DayType,
  opts?: HoursForDayTypeOpts
): number[] {
  const hours = [...(index?.hoursByDayType?.[dayType] ?? [])];
  const latest = opts?.latestUpdated;
  if (latest) {
    const parsed = madridHourAndDayTypeFromIso(latest);
    if (parsed && parsed.dayType === dayType && !hours.includes(parsed.hour)) {
...
export async function loadHourlyViewData(
  ...
  const cacheKey = hourlyViewCacheKey(hour, dayType);
  const cached = hourlyViewCache.get(cacheKey);
  if (cached) return cached;

  const inflight = hourlyViewInflight.get(cacheKey);
  if (inflight) return inflight;

  const promise = (async (): Promise<HourlyViewData> => {
    ...
    if (!byStationIdx.size || !idOrder.length) {
      const result = { barris, stations: null };
      hourlyViewCache.set(cacheKey, result);
      return result;
    }
    ...
    const result = { barris, stations: stations.length ? stations : null };
    hourlyViewCache.set(cacheKey, result);
    return result;
  })().finally(() => {
    hourlyViewInflight.delete(cacheKey);
  });

  hourlyViewInflight.set(cacheKey, promise);
  return promise;
}

/** Precarrega snapshots i mitjanes per al replay hora a hora. */
export async function preloadReplayHistory(
  index: HistoryIndex | null,
  baseStations: Station[],
  stationIds: string[] | null
): Promise<void> {
  ...
  const keys = [...new Set(index.files.map((f) => f.key))];
  await Promise.all(keys.map((key) => loadHourlySnapshot(key)));

  await Promise.all(
    REPLAY_DAY_TYPE_ORDER.flatMap((dayType) =>
      hoursForDayType(index, dayType).map((hour) =>
        loadHourlyViewData(index, hour, dayType, baseStations, stationIds)
      )
    )
  );
}
    ...
    if (key === "pct_oos_anchors") {
      ...
      value =
        ...
        (p.pct_bikes !== undefined && p.pct_oos_anchors !== undefined
          ...
          : undefined);
    } else if (key === "bikes_out_of_service") {
      value = p.bikes_out_of_service;
    } else {
      value = p[key];
    }
...
export function hourlyAverage(
  ...
  if (key === "bikes_out_of_service") {
    const samples = bucket.samples.filter((s) => s.bikes_out_of_service !== undefined);
    if (!samples.length) return null;
    return (
      samples.reduce((sum, s) => sum + (s.bikes_out_of_service ?? 0), 0) / samples.length
    );
  }
  return null;
}
...
export async function barriHistAveragesAtHour(
  ...
  return {
    ...
    bikes_out_of_service: avg(bikes_out_of_service),
